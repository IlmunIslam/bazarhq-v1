...
// Mirrors the web storefront homepage (frontend/app/sites/[shop]/page.tsx):
// category pills + debounced search + cursor-paginated product grid.
...
export default function StoreHomeScreen() {
  ...
  useEffect(() => {
    api.get<ShopResponse>(`/storefront/${ACTIVE_SHOP}`).then(res => {
      if (res.success) {
  ...
  const fetchProducts = useCallback(
    async (opts: { category?: string; search?: string; cursor?: string }) => {
      ...
      return api.get<ProductsResponse>(`/storefront/${ACTIVE_SHOP}/products?${params}`);
    },
    [],
  );
