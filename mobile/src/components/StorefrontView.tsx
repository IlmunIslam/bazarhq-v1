...
// Shared storefront-home rendering for a single shop, driven by its subdomain.
// Mirrors the web storefront homepage: shop hero + category pills + debounced
// search + cursor-paginated product grid. Used by both the CUSTOMER shop stack
// (interactive — products navigate to detail via `onProductPress`) and the
// MERCHANT store preview (read-only — no `onProductPress`, so cards don't tap).
// It only READS public /storefront endpoints, so it never mutates anything.
...
export interface StorefrontViewProps {
  subdomain: string;
  /** When provided, product cards are tappable and call this with the slug.
   *  When omitted, the grid is read-only (merchant preview). */
  onProductPress?: (slug: string) => void;
  /** Set the native header title to the shop's name once loaded (default true).
   *  The merchant preview passes false so its own "Store preview" title stands. */
  showShopTitle?: boolean;
}

export default function StorefrontView({ subdomain, onProductPress, showShopTitle = true }: StorefrontViewProps) {
  ...
  return (
    <>
      {showShopTitle && <Stack.Screen options={{ title: shopName || 'Store' }} />}
      <FlatList
        ...
        ListFooterComponent={
          loading || loadingMore ? <ActivityIndicator style={styles.footer} /> : <View style={styles.footer} />
        }
        ListEmptyComponent={
          !loading ? (
...
function ProductCard({
  ...
  // Interactive (customer) vs read-only (merchant preview).
  if (onPress) {
    return (
      <Pressable style={styles.card} onPress={() => onPress(product.slug)}>
        {body}
      </Pressable>
    );
  }
